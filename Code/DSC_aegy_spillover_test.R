...
source("email_notification.R") #--!!!!!confidential!!!!!
...
check_continuity <- function(x){
  xmin <- min(x)
  xmax <- max(x)
  xlength <- length(x)
  return (xlength == xmax - xmin + 1)
}
...
while (length(treatment_ids) != 0){
  ...
  time.start <- brief.release.info %>% filter(Sector_ID == x & type == "NN") %>% select(Eweek_tot) %>% unlist() %>% unname()
  time.end  <- brief.release.info %>% filter(Sector_ID == x & type == "NA.") %>% select(end_on) %>% unlist() %>% unname()
  treat_surveillance = total_surveillance %>% filter(Eweek_tot >= time.start & Eweek_tot <= time.end & Sector_ID == x)
  treat_weeks <- treat_surveillance$Eweek_tot |> unique() |> unlist() |> length()
  is.continuous <- treat_surveillance$Eweek_tot |> unique() |> unlist() |> check_continuity()
  if (is.continuous){
    if (treat_surveillance$Eweek_tot |> unique() |> unlist() |> min() > time.start) time.start = treat_surveillance$Eweek_tot |> unique() |> unlist() |> min()
    if (treat_surveillance$Eweek_tot |> unique() |> unlist() |> max() > time.end) time.end = treat_surveillance$Eweek_tot |> unique() |> unlist() |> max()
    ...
    print(paste("-------",x,"-------"))
    p <- sector_DSC(treatment_id = x, quantile_type = q_type, B= B,lambda.ind = l.ind, interval.eweek = wks, space.agg = space.agg, beta.ind = beta.ind, time.lag = time.lag, use.cores = 20, time.weight.ind = time.weight.ind, trim = trim.ind, treat_surveillance = treat_surveillance, control_surveillance = control_surveillance)
    system("clear")
  }
