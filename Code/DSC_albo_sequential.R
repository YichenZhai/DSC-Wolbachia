...
while (length(treatment_ids) != 0){
    x = treatment_ids[1]
    print(paste("-------",x,"-------"))
    p <- sector_DSC(treatment_id = x, quantile_type = q_type, B= B,lambda.ind = l.ind, interval.eweek = wks, space.agg = space.agg, beta.ind = beta.ind, time.lag = time.lag, use.cores = 20, time.weight.ind = time.weight.ind, trim = trim.ind, treat_surveillance = treat_surveillance_original, control_surveillance = control_surveillance_original)
    system("clear")
